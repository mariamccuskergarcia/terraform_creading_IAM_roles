...
locals {
  sitename       = format("%s%s%ssweb", var.prefix, var.cloudid, var.environment) # will be our new local
  aad_group_name = format("%s Project Users", var.cohortid)                       # aad=azure active directory
